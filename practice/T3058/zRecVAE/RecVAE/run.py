...
parser.add_argument('--dataset', type=str, default= "/opt/ml/output")
...
def inference(model, data_in, k, samples_perc_per_epoch=1, batch_size=500):
    model.eval()
    ...
    result = np.argpartition(ratings_pred, -k).iloc[:,-k:]
    users, items = list(), list()
    item_columns = ratings_pred.columns
...
for epoch in range(args.n_epochs):
    ...
    if args.not_alternating:
        ...
        run(opts=[optimizer_encoder], n_epochs=args.n_enc_epochs, dropout_rate=0.2, **learning_kwargs)
        model.update_prior()
        run(opts=[optimizer_decoder], n_epochs=args.n_dec_epochs, dropout_rate=0, **learning_kwargs)

    train_scores.append(
        evaluate(model, train_data, train_data, metrics, 1)[0]
    )
    valid_scores.append(
        evaluate(model, valid_in_data, valid_out_data, metrics, 1)[0]
    )
    
    if valid_scores[-1] > best_recall:
        best_recall = valid_scores[-1]
        model_best.load_state_dict(deepcopy(model.state_dict()))
        

    print(f'epoch {epoch} | valid recall@10: {valid_scores[-1]:.4f} | ' +
          f'best valid: {best_recall:.4f} | train recall@10: {train_scores[-1]:.4f}')


    
test_metrics = [{'metric': ndcg, 'k': 10}, {'metric': recall, 'k': 10}, {'metric': recall, 'k': 20}]
...
inference(model, test_in_data,k=10)
